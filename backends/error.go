...
// Put stores an object in the cache, potentially returning an error.
func (e *Error) Put(actionID, outputID []byte, body io.Reader, bodySize int64) (string, error) {
	if e.shouldError() {
		e.putErrors.Add(1)
...
// Get retrieves an object from the cache, potentially returning an error.
func (e *Error) Get(actionID []byte) ([]byte, string, int64, *time.Time, bool, error) {
	if e.shouldError() {
		e.getErrors.Add(1)
		return nil, "", 0, nil, false, fmt.Errorf("error backend: simulated Get error (error rate: %.2f%%)", e.errorRate*100)
	}
